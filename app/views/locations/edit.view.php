        ...
        <div id="main-content" class="pt-0">
            <div class="page-heading">
                <div class="page-title">
                    <div class="row">
                        <div class="col-12 col-md-6 order-md-1 order-last">
                            <h3>Edit Location</h3>
                            <p class="text-subtitle text-muted">Edit the existing record</p>
                        </div>
                ...
                <section class="section">
                    <div class="card">
                        <div class="card-body">
                            <form method="post" action="<?= url('locations/update') ?>" class="form form-vertical">
                                <div class="form-body">
                                    <div class="row">
                                        <div class="col-12">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select id="state" class="form-select <?php if(error('state')) echo 'is-invalid'; ?>">
                                                    ...
                                                    <?php foreach ($states as $state): ?>
                                                        <option value="<?= $state['state_id'] ?>" <?php if($state['state_id'] === $location['state_id']) echo 'selected' ?>><?= $state['state_name'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="city_id" class="form-label">City</label>
                                                <select id="city" name="city_id" class="form-select <?php if(error('city')) echo 'is-invalid'; ?>">
                                                    <option value="">Select City</option>
                                                    <?php foreach ($cities as $city): ?>
                                                        <option value="<?= $city['city_id'] ?>" <?php if($city['city_id'] === $location['city_id']) echo 'selected' ?>><?= $city['city_name'] ?></option>
                                                    <?php endforeach; ?>
                                                </select>
                                            </div>
                                            <div class="form-group">
                                                <label for="location_name" class="form-label">Location</label>
                                                <input type="text" name="location_name" value="<?= $location['location_name'] ?>" class="form-control <?php if(error('location_name')) echo 'is-invalid'; ?>"
